<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" href="css/swiper-bundle.min.css" />
  <script src="js/swiper-bundle.min.js"></script>
  <link rel="stylesheet" href="css/style.css" />
  <link href="/css/aos.css" rel="stylesheet">
  <title>Welcome to FashionExpertise.agency — Fashion Design & Consulting </title>
  <link rel="apple-touch-icon" sizes="180x180" href="/media/icons/apple-touch-icon.png">
  <link rel="icon" type="image/png" sizes="32x32" href="/media/icons/favicon-32x32.png">
  <link rel="icon" type="image/png" sizes="16x16" href="/media/icons/favicon-16x16.png">
  <link rel="manifest" href="/media/icons/site.webmanifest">
</head>
<body>
  <div class="loader">
    <div class="con-1">
      <div class="bc"></div>
      <div class="con">
        <img src="media/logo.svg" alt="" />
      </div>
      <div>Please wait</div>
    </div>
  </div>
  <section id="Header-section">
    <header class="header nm">
      <div class="logo">
        <a href="">
          <img src="media/logo.svg" alt="" />
        </a>
      </div>
      <ul class="navBar">
        <li>
          <a href="#whyus">Why us</a>
        </li>
        <li>
          <a href="#whatwedo">What we do</a>
        </li>
        <li>
          <a href="#ourservices">Our services</a>
        </li>
        <li>
          <a href="#ourclients">Our clients</a>
        </li>
        <li>
          <a href="contact.php">Let's get in touch</a>
        </li>
      </ul>
      <input type="checkbox" id="mobile-menu-toggle" class="mobile-menu-checkbox" hidden />
      <label for="mobile-menu-toggle" class="mobile-in">
        <span></span>
        <span></span>
        <span></span>
      </label>
    </header>
    <div class="body">
      <h1 class="nm">
        We believe , Creativity and originality are the key for a better
        fashion industry world.
      </h1>
      <div class="learnmore nm">
        <a class="learnMore-btn" href="#whyus">
          <div class="line"></div>
          <span>Learn More</span>
        </a>
      </div>
    </div>
    <div class="container">
      <ul class="social-media">
        <li class="instagram nm">
          <a href="">
            <svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" class="icon" fill="white">
              <path
                d="M512 378.7c-73.4 0-133.3 59.9-133.3 133.3S438.6 645.3 512 645.3 645.3 585.4 645.3 512 585.4 378.7 512 378.7zM911.8 512c0-55.2.5-109.9-2.6-165-3.1-64-17.7-120.8-64.5-167.6-46.9-46.9-103.6-61.4-167.6-64.5-55.2-3.1-109.9-2.6-165-2.6-55.2 0-109.9-.5-165 2.6-64 3.1-120.8 17.7-167.6 64.5C132.6 226.3 118.1 283 115 347c-3.1 55.2-2.6 109.9-2.6 165s-.5 109.9 2.6 165c3.1 64 17.7 120.8 64.5 167.6 46.9 46.9 103.6 61.4 167.6 64.5 55.2 3.1 109.9 2.6 165 2.6 55.2 0 109.9.5 165-2.6 64-3.1 120.8-17.7 167.6-64.5 46.9-46.9 61.4-103.6 64.5-167.6 3.2-55.1 2.6-109.8 2.6-165zM512 717.1c-113.5 0-205.1-91.6-205.1-205.1S398.5 306.9 512 306.9 717.1 398.5 717.1 512 625.5 717.1 512 717.1zm213.5-370.7c-26.5 0-47.9-21.4-47.9-47.9s21.4-47.9 47.9-47.9 47.9 21.4 47.9 47.9a47.84 47.84 0 0 1-47.9 47.9z" />
            </svg>
          </a>
        </li>
        <li class="likendin nm">
          <a href="">
            <svg class="facebook" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 310 310"
              style="enable-background: new 0 0 310 310" xml:space="preserve">
              <g id="XMLID_834_">
                <path id="XMLID_835_"
                  d="M81.703,165.106h33.981V305c0,2.762,2.238,5,5,5h57.616c2.762,0,5-2.238,5-5V165.765h39.064 c2.54,0,4.677-1.906,4.967-4.429l5.933-51.502c0.163-1.417-0.286-2.836-1.234-3.899c-0.949-1.064-2.307-1.673-3.732-1.673h-44.996 V71.978c0-9.732,5.24-14.667,15.576-14.667c1.473,0,29.42,0,29.42,0c2.762,0,5-2.239,5-5V5.037c0-2.762-2.238-5-5-5h-40.545 C187.467,0.023,186.832,0,185.896,0c-7.035,0-31.488,1.381-50.804,19.151c-21.402,19.692-18.427,43.27-17.716,47.358v37.752H81.703 c-2.762,0-5,2.238-5,5v50.844C76.703,162.867,78.941,165.106,81.703,165.106z" />
              </g>
            </svg>
          </a>
        </li>
        <li class="facebook nm">
          <a href="">
            <svg fill="white" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <g data-name="Layer 2">
                <g data-name="linkedin">
                  <rect width="24" height="24" transform="rotate(180 12 12)" opacity="0" />
                  <path
                    d="M15.15 8.4a5.83 5.83 0 0 0-5.85 5.82v5.88a.9.9 0 0 0 .9.9h2.1a.9.9 0 0 0 .9-.9v-5.88a1.94 1.94 0 0 1 2.15-1.93 2 2 0 0 1 1.75 2v5.81a.9.9 0 0 0 .9.9h2.1a.9.9 0 0 0 .9-.9v-5.88a5.83 5.83 0 0 0-5.85-5.82z" />
                  <rect x="3" y="9.3" width="4.5" height="11.7" rx=".9" ry=".9" />
                  <circle cx="5.25" cy="5.25" r="2.25" />
                </g>
              </g>
            </svg>
          </a>
        </li>
      </ul>
    </div>
    <div class="scroll nm">
      <span>Scroll</span>
      <svg width="12" height="36" viewBox="0 0 12 36" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M6.07117 35.9957L11.8447 25.9957H0.297665L6.07117 35.9957ZM7.07117 26.9957L7.07117 0.99573H5.07117L5.07117 26.9957H7.07117Z"
          fill="white" />
      </svg>
    </div>
    <div class="video">
      <video src="media/video.mp4" muted loop="true"></video>
    </div>
  </section>
  <section id="whyus" data-aos="fade">
    <div class="container-section">
      <div class="image-container">
        <img src="media/images/why-us.jpeg" alt="" />
      </div>
      <div class="text-container">
        <div class="top-head">
          <h2>Why Fashion expertise agency ?</h2>
          <div class="line"></div>
        </div>
        <p class="description">
          <b>Fashion expertise agency</b> Core Work is Fashion Design, we
          offer more than one service in creation, Labeling, researches and
          development, Textile and clothing, etc. <br />
          I'ts is a universe specialized in creation, with an Automatized
          system for Research and development, Sketching and creation, it
          exists to facilitate the designing and give an access to all
          research in an efficient way which is Smart for us to spread
        </p>
        <a href="contact.php" class="contact">
          <span>Let’s discuss Business</span>
          <span class="line"></span>
        </a>
      </div>
    </div>
  </section>
  <section id="whatwedo" data-aos="fade">
    <div class="top-head">
      <h2>What FEA do ?</h2>
      <div class="line"></div>
    </div>
    </div>
    <div class="wrapper">
      <div class="home-hero">
        <div class="womenwear serv">
          <h3>Womenwear</h3>
        </div>
        <div class="kidswear serv">
          <h3>Kidswear</h3>
        </div>
        <div class="denimwear serv">
          <h3>Denimwear</h3>
        </div>
        <div class="menswear serv">
          <h3>Menswear</h3>
        </div>
        <div class="classicwear serv">
          <h3>Classicwear</h3>
        </div>
        <div class="casualwear serv">
          <h3>Casualwear</h3>
        </div>
      </div>
    </div>
  </section>
  <section id="ourservices" data-aos="fade">
    <div class="top-head">
      <h2>Our services</h2>
      <div class="line"></div>
    </div>
    <p>We offer more than one service destined to all the providers of fashion and apparel manufacturers,
      in order to be informed of the latest trends ideas and creations divided in personalized services
      depending on the client’s need, starting from Analyzing,
      reporting , creating, conception and finally shipment.
    </p>
    <div class="grid">
      <a class="item" href="services/we-are-creation.php">
        <img src="media/images/conc.png" alt="">
        <div class="text-info">
          <h2>We're creation</h2>
          <h3>Brand conception</h3>
        </div>
      </a>
      <a class="item" href="services/Just-in-time-creation.php">
        <img src="media/images/brand-1.png" alt="">
        <div class="text-info">
          <h2>Just in time creation</h2>
          <h3>Reaserches and developement</h3>
        </div>
      </a>
      <a class="item" href="services/we-are-innovation.php">
        <img src="media/images/ref.png" alt="">
        <div class="text-info">
          <h2>We're innovation</h2>
          <h3>Brand refreshement</h3>
        </div>
      </a>
      <a class="item" href="services/we-are-personnalization.php">
        <img src="media/images/Uniforms-personalized.png" alt="">
        <div class="text-info">
          <h2>We're personnalization</h2>
          <h3>Uniforms personalized service</h3>
        </div>
      </a>

    </div>
  </section>
  <section id="quote" data-aos="fade">
    <div class="text-display">
      <div class="qut">
        <svg width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M26.968 30.7583C29.8583 30.8567 32.2848 28.5205 32.3859 25.5396C32.4868 22.5606 30.2242 20.0629 27.3339 19.9646C22.1008 19.7864 25.9426 9.47297 32.9224 9.71056L33.0078 7.19201C20.5508 6.76618 14.8646 30.3463 26.968 30.7583V30.7583ZM11.8881 30.245C14.7766 30.3433 17.2031 28.0071 17.3041 25.0263C17.4051 22.0472 15.1425 19.5496 12.254 19.4512C7.01906 19.2731 10.8608 8.9596 17.8407 9.19719L17.9261 6.67864C5.4708 6.25287 -0.215393 29.833 11.8881 30.245V30.245Z"
            fill="white" fill-opacity="0.82" />
        </svg>
      </div>
      <h1>
        Created with passion, Fashion expertise agency is based on working witha new approach in the Management of
        designing in Fashion Industry.
      </h1>
      <div class="qut">
        <svg width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M26.968 30.7583C29.8583 30.8567 32.2848 28.5205 32.3859 25.5396C32.4868 22.5606 30.2242 20.0629 27.3339 19.9646C22.1008 19.7864 25.9426 9.47297 32.9224 9.71056L33.0078 7.19201C20.5508 6.76618 14.8646 30.3463 26.968 30.7583V30.7583ZM11.8881 30.245C14.7766 30.3433 17.2031 28.0071 17.3041 25.0263C17.4051 22.0472 15.1425 19.5496 12.254 19.4512C7.01906 19.2731 10.8608 8.9596 17.8407 9.19719L17.9261 6.67864C5.4708 6.25287 -0.215393 29.833 11.8881 30.245V30.245Z"
            fill="white" fill-opacity="0.82" />
        </svg>
      </div>
    </div>
    <a href="contact.php" class="contact-us">
      Contact us
      <div class="line"></div>
    </a>
  </section>
  <section id="ourclients" data-aos="fade">
    <div class="top-head">
      <h2>Our clients</h2>
      <div class="line"></div>
    </div>
    <p class="description">
      Apparel manufacturers / Present labels with attention of refreshing their brands / Investors willing to enter the
      world of fashion / Movie and Music producers / Stuff for Hotels / Restaurants / Malls…
    </p>
    </div>
    <div class="clients">
      <div class="client">
        <img src="media/images/clients/filmod.png" alt="">
      </div>
      <div class="client">
        <img src="media/images/clients/new-wash.png" alt="">
      </div>
     
      <div class="client">
        <img src="media/images/clients/med-consortium.png" alt="">
      </div>
      <div class="client">
        <img src="media/images/clients/installator.png" alt="">
      </div>
      <div class="client">
        <img src="media/images/clients/blue fingers.png" alt="">
      </div>
     
      <div class="client">
        <img src="media/images/clients/ipom.jpeg" alt="">
      </div>
      <div class="client ">
        <img src="media/images/clients/mastertex.png" alt="">
      </div>
      <div class="client">
        <img class="s" src="media/images/clients/casual-paint.png" alt="">
      </div>
       <div class="client">
        <img class="s" src="media/images/clients/bluefve.png" alt="">
      </div>
    </div>
    <div class="testimonial-quotes">
      <h2 class="mb">
        <span>Our partners</span>
      </h2>
      <div class="partners">
        <a href="https://www.evlox.com/" target="_blank" class="partner">
          <img src="media/images/partners/EVLOX.png" alt="">
        </a>
        <a href="#" target="_blank" class="partner">
          <img src="media/images/partners/almaa.png" alt="">
        </a>
        <div class="partner">
          <img src="media/images/partners/wasssh.png" alt="">

        </div>
      </div>
    </div>
    <div class="testimonial-quotes">
      <h2>
        Testimonial and <br>
        <span>quotes</span>
      </h2>
      <div class="swiper mySwiper">
        <div class="swiper-wrapper">
          <div class="swiper-slide">
            <p>
              “ Our expertised team will
              organize looks for every scene
              that you can follow from the
              ﬁle that will be delivered
              with your clothes ! ”
            </p>

          </div>
          <div class="swiper-slide">
            <p>
              “We know how to meet with the
              clients needs, our expertise in
              marketing and communication is
              linked
              to our love to fashion to make
              sure you set what in your clients
              mind!”
            </p>
          </div>
          <div class="swiper-slide">
            <p>
              “We know how to meet with the
              clients needs, our expertise in
              marketing and communication is
              linked
              to our love to fashion to make
              sure you set what in your clients
              mind!”
            </p>
          </div>
        </div>
        <div class="swiper-pagination"></div>
      </div>
    </div>
  </section>

  <section id="contactus" data-aos="fade">
    <div class="top-head">
      <h2>Contact us</h2>
      <div class="line"></div>
    </div>
    <div class="contact">
      <div class="image-section c798">
        <div class="photographer">
          <a class="name" href="https://unsplash.com/@korie" target="_blank">
            <small>Taken by <b>@korie cull</b></small>
          </a>
        </div>
      </div>
      <div class="contact-info c798">
        <h2>
          let’s get in touch
          <span class="line"></span>
        </h2>
        <div class="links">
          <div class="link email">
            <h3>
              <a href="">
                <img src="media/icons/email.svg" alt="">
                hello@fashionexpertise.agency
              </a>
            </h3>
            <span>Beyond creativity</span>
          </div>
          <div class="link tel">
            <h3>
              <a href="tel:+212664300097">
                <img src="media/icons/phone.svg" alt="">
                +212664300097
              </a>
            </h3>
          </div>
          <div class="link social-media">
            <div class="facebook">
              <svg class="facebook" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 310 310"
                style="enable-background: new 0 0 310 310" xml:space="preserve">
                <g id="XMLID_834_">
                  <path id="XMLID_835_"
                    d="M81.703,165.106h33.981V305c0,2.762,2.238,5,5,5h57.616c2.762,0,5-2.238,5-5V165.765h39.064 c2.54,0,4.677-1.906,4.967-4.429l5.933-51.502c0.163-1.417-0.286-2.836-1.234-3.899c-0.949-1.064-2.307-1.673-3.732-1.673h-44.996 V71.978c0-9.732,5.24-14.667,15.576-14.667c1.473,0,29.42,0,29.42,0c2.762,0,5-2.239,5-5V5.037c0-2.762-2.238-5-5-5h-40.545 C187.467,0.023,186.832,0,185.896,0c-7.035,0-31.488,1.381-50.804,19.151c-21.402,19.692-18.427,43.27-17.716,47.358v37.752H81.703 c-2.762,0-5,2.238-5,5v50.844C76.703,162.867,78.941,165.106,81.703,165.106z">
                  </path>
                </g>
              </svg>
              <a href="">
                <h3>fashionExpertiseagency</h3>
              </a>
            </div>
            <div class="instagram">
              <svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" class="icon">
                <path
                  d="M512 378.7c-73.4 0-133.3 59.9-133.3 133.3S438.6 645.3 512 645.3 645.3 585.4 645.3 512 585.4 378.7 512 378.7zM911.8 512c0-55.2.5-109.9-2.6-165-3.1-64-17.7-120.8-64.5-167.6-46.9-46.9-103.6-61.4-167.6-64.5-55.2-3.1-109.9-2.6-165-2.6-55.2 0-109.9-.5-165 2.6-64 3.1-120.8 17.7-167.6 64.5C132.6 226.3 118.1 283 115 347c-3.1 55.2-2.6 109.9-2.6 165s-.5 109.9 2.6 165c3.1 64 17.7 120.8 64.5 167.6 46.9 46.9 103.6 61.4 167.6 64.5 55.2 3.1 109.9 2.6 165 2.6 55.2 0 109.9.5 165-2.6 64-3.1 120.8-17.7 167.6-64.5 46.9-46.9 61.4-103.6 64.5-167.6 3.2-55.1 2.6-109.8 2.6-165zM512 717.1c-113.5 0-205.1-91.6-205.1-205.1S398.5 306.9 512 306.9 717.1 398.5 717.1 512 625.5 717.1 512 717.1zm213.5-370.7c-26.5 0-47.9-21.4-47.9-47.9s21.4-47.9 47.9-47.9 47.9 21.4 47.9 47.9a47.84 47.84 0 0 1-47.9 47.9z">
                </path>
              </svg>
              <a href="">
                <h3>instagram.com/FEA</h3>
              </a>
            </div>
            <div class="likendin">
              <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <g data-name="Layer 2">
                  <g data-name="linkedin">
                    <rect width="24" height="24" transform="rotate(180 12 12)" opacity="0"></rect>
                    <path
                      d="M15.15 8.4a5.83 5.83 0 0 0-5.85 5.82v5.88a.9.9 0 0 0 .9.9h2.1a.9.9 0 0 0 .9-.9v-5.88a1.94 1.94 0 0 1 2.15-1.93 2 2 0 0 1 1.75 2v5.81a.9.9 0 0 0 .9.9h2.1a.9.9 0 0 0 .9-.9v-5.88a5.83 5.83 0 0 0-5.85-5.82z">
                    </path>
                    <rect x="3" y="9.3" width="4.5" height="11.7" rx=".9" ry=".9"></rect>
                    <circle cx="5.25" cy="5.25" r="2.25"></circle>
                  </g>
                </g>
              </svg>
              <a href="">
                <h3>fashionExpertise</h3>
              </a>
            </div>
          </div>
          <div class="link">
            <a href="contact.php" class="contact">
              <span>
                Send us a message now
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 495.003 495.003"
                  style="enable-background:new 0 0 495.003 495.003;" xml:space="preserve">
                  <g>
                    <path
                      d="M164.711,456.687c0,2.966,1.647,5.686,4.266,7.072c2.617,1.385,5.799,1.207,8.245-0.468l55.09-37.616 l-67.6-32.22V456.687z" />
                    <path
                      d="M492.431,32.443c-1.513-1.395-3.466-2.125-5.44-2.125c-1.19,0-2.377,0.264-3.5,0.816L7.905,264.422 c-4.861,2.389-7.937,7.353-7.904,12.783c0.033,5.423,3.161,10.353,8.057,12.689l125.342,59.724l250.62-205.99L164.455,364.414 l156.145,74.4c1.918,0.919,4.012,1.376,6.084,1.376c1.768,0,3.519-0.322,5.186-0.977c3.637-1.438,6.527-4.318,7.97-7.956 L494.436,41.257C495.66,38.188,494.862,34.679,492.431,32.443z" />
                  </g>
                </svg>
              </span>
              <span class="line"></span>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
  <footer>
    <div class="container-footer">
      <div class="logo section">
        <div class="center">
          <img src="media/logo.svg" alt="">
        </div>
      </div>
      <div class="about-us section">
        <div class="center">
          <div class="tit">
            <h3>About us</h3>
            <div class="line"></div>
          </div>
          <p class="description">
            <b>FashionExpertise Agency</b> Have mush planned to the future,working with great clients and continued
            Fashion
            industry.If you’d like to join our team.
          </p>
          <div class="social-media">
            <a href="" class="sc">
              <svg width="15" height="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M4.24194 7.989H5.88618V14.7581C5.88618 14.8917 5.99447 15 6.12811 15H8.91598C9.04963 15 9.15792 14.8917 9.15792 14.7581V8.02089H11.0481C11.171 8.02089 11.2744 7.92866 11.2885 7.80658L11.5755 5.31455C11.5834 5.24598 11.5617 5.17732 11.5158 5.12589C11.4699 5.0744 11.4042 5.04494 11.3352 5.04494H9.15802V3.48281C9.15802 3.0119 9.41156 2.77311 9.91169 2.77311C9.98297 2.77311 11.3352 2.77311 11.3352 2.77311C11.4689 2.77311 11.5772 2.66477 11.5772 2.53118V0.243726C11.5772 0.110081 11.4689 0.00179032 11.3352 0.00179032H9.37339C9.35955 0.0011129 9.32882 0 9.28353 0C8.94313 0 7.75992 0.0668225 6.82527 0.926661C5.78969 1.8795 5.93365 3.02037 5.96805 3.21818V5.04489H4.24194C4.10829 5.04489 4 5.15318 4 5.28682V7.74702C4 7.88066 4.10829 7.989 4.24194 7.989Z" />
              </svg>
            </a>
            <a href="" class="sc">
              <svg width="16" height="17" viewBox="0 0 16 17" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M13.9647 5.8825C13.9579 5.34602 13.8634 4.81488 13.6853 4.31283C13.5309 3.88935 13.295 3.50476 12.9928 3.18362C12.6905 2.86249 12.3286 2.61187 11.93 2.44779C11.4636 2.26175 10.9708 2.16115 10.4727 2.15029C9.83133 2.11983 9.628 2.11133 8 2.11133C6.372 2.11133 6.16333 2.11133 5.52667 2.15029C5.02877 2.16123 4.53623 2.26183 4.07 2.44779C3.67137 2.61176 3.30935 2.86233 3.00709 3.18348C2.70483 3.50463 2.46899 3.88929 2.31467 4.31283C2.13922 4.80805 2.04475 5.33149 2.03533 5.86054C2.00667 6.54266 1.998 6.7587 1.998 8.48845C1.998 10.2182 1.998 10.4392 2.03533 11.1164C2.04533 11.6462 2.13933 12.169 2.31467 12.6655C2.46925 13.0889 2.70527 13.4734 3.00763 13.7944C3.30999 14.1154 3.67204 14.3659 4.07067 14.5298C4.53562 14.7233 5.02825 14.8311 5.52733 14.8486C6.16933 14.879 6.37267 14.8882 8.00067 14.8882C9.62867 14.8882 9.83734 14.8882 10.474 14.8486C10.9721 14.8382 11.4649 14.7378 11.9313 14.5518C12.3298 14.3875 12.6917 14.1368 12.9939 13.8157C13.2961 13.4946 13.5321 13.1101 13.6867 12.6867C13.862 12.1909 13.956 11.6682 13.966 11.1376C13.9947 10.4562 14.0033 10.2402 14.0033 8.5097C14.002 6.77995 14.002 6.56037 13.9647 5.8825ZM7.996 11.7595C6.29333 11.7595 4.914 10.294 4.914 8.48491C4.914 6.67583 6.29333 5.21029 7.996 5.21029C8.8134 5.21029 9.59732 5.55529 10.1753 6.1694C10.7533 6.78351 11.078 7.61643 11.078 8.48491C11.078 9.3534 10.7533 10.1863 10.1753 10.8004C9.59732 11.4145 8.8134 11.7595 7.996 11.7595ZM11.2007 5.85274C11.1063 5.85284 11.0128 5.83315 10.9255 5.79481C10.8383 5.75647 10.7591 5.70023 10.6923 5.6293C10.6255 5.55838 10.5726 5.47417 10.5365 5.38148C10.5004 5.2888 10.4819 5.18946 10.482 5.08916C10.482 4.98893 10.5006 4.88969 10.5367 4.79709C10.5728 4.70449 10.6257 4.62035 10.6924 4.54948C10.7591 4.4786 10.8383 4.42239 10.9254 4.38403C11.0126 4.34567 11.106 4.32593 11.2003 4.32593C11.2947 4.32593 11.3881 4.34567 11.4752 4.38403C11.5624 4.42239 11.6416 4.4786 11.7083 4.54948C11.775 4.62035 11.8279 4.70449 11.864 4.79709C11.9001 4.88969 11.9187 4.98893 11.9187 5.08916C11.9187 5.51133 11.5973 5.85274 11.2007 5.85274Z" />
                <path
                  d="M7.996 10.6126C9.10168 10.6126 9.998 9.6603 9.998 8.48552C9.998 7.31075 9.10168 6.3584 7.996 6.3584C6.89033 6.3584 5.994 7.31075 5.994 8.48552C5.994 9.6603 6.89033 10.6126 7.996 10.6126Z" />
              </svg>

            </a>
            <a href="" class="sc">
              <svg width="12" height="12" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M2.79381 3.85254H0.384773C0.277856 3.85254 0.191223 3.93933 0.191223 4.04636V11.7963C0.191223 11.9033 0.277856 11.9901 0.384773 11.9901H2.79381C2.90073 11.9901 2.98736 11.9033 2.98736 11.7963V4.04636C2.98736 3.93933 2.90073 3.85254 2.79381 3.85254Z" />
                <path
                  d="M1.58967 0C0.713116 0 0 0.713322 0 1.59011C0 2.46728 0.713116 3.18088 1.58967 3.18088C2.46552 3.18088 3.17805 2.46724 3.17805 1.59011C3.17809 0.713322 2.46552 0 1.58967 0Z" />
                <path
                  d="M8.92086 3.66113C7.95333 3.66113 7.2381 4.07765 6.80429 4.55091V4.04757C6.80429 3.94054 6.71766 3.85375 6.61074 3.85375H4.30371C4.1968 3.85375 4.11017 3.94054 4.11017 4.04757V11.7975C4.11017 11.9045 4.1968 11.9913 4.30371 11.9913H6.70744C6.81435 11.9913 6.90098 11.9045 6.90098 11.7975V7.96307C6.90098 6.67097 7.25145 6.16758 8.1509 6.16758C9.13047 6.16758 9.20832 6.97456 9.20832 8.02955V11.7975C9.20832 11.9046 9.29495 11.9913 9.40186 11.9913H11.8065C11.9134 11.9913 12 11.9046 12 11.7975V7.54656C12 5.62524 11.6342 3.66113 8.92086 3.66113Z" />
              </svg>

            </a>
          </div>
        </div>
      </div>
      <div class="our-links section">
        <div class="center">
          <div class="tit">
            <h3>Our links</h3>
            <div class="line"></div>
          </div>
          <ul>
            <li>
              <a href="#whyus">Why us</a>
            </li>
            <li>
              <a href="#whatwedo">What we do</a>
            </li>
            <li>
              <a href="#ourservices">Our services</a>
            </li>
            <li>
              <a href="#ourclients">Our clients</a>
            </li>
            <li>
              <a href="contact.php">Let's get in touch</a>
            </li>
          </ul>
        </div>
      </div>
      <div class="our-services section">
        <div class="center">
          <div class="tit">
            <h3>Our services</h3>
            <div class="line"></div>
          </div>
          <ul>
            <li>
              <a href="services/we-are-creation.php">Brand conception </a>
            </li>
            <li>
              <a href="services/Just-in-time-creation.php">Reaserches and developement</a>
            </li>
            <li>
              <a href="services/we-are-innovation.php">Brand refreshement</a>
            </li>
            <li>
              <a href="services/we-are-personnalization.php">Uniforms personalized service</a>
            </li>
          </ul>
        </div>
      </div>
      <div class="contact section">
        <div class="center">
          <div class="tit">
            <h3>Contact us</h3>
            <div class="line"></div>
          </div>
          <ul>
            <li>
              <a href="tel:+212661173921">
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 287.32 287.32"
                  style="enable-background:new 0 0 287.32 287.32;" xml:space="preserve">
                  <path
                    d="M267.749,191.076c-14.595-11.729-27.983-17.431-40.93-17.431c-18.729,0-32.214,11.914-44.423,24.119 c-1.404,1.405-3.104,2.06-5.349,2.06c-10.288,0.001-28.387-12.883-53.794-38.293c-29.89-29.892-41.191-48.904-33.592-56.506 c20.6-20.593,27.031-41.237-4.509-80.462C73.861,10.51,62.814,3.68,51.38,3.68c-15.42,0-27.142,12.326-37.484,23.202 c-1.788,1.88-3.477,3.656-5.133,5.312c-11.689,11.688-11.683,37.182,0.017,68.2c12.837,34.033,38.183,71.055,71.37,104.247 c25.665,25.663,53.59,46.403,80.758,60.328c23.719,12.158,46.726,18.672,64.783,18.672c0.002,0,0.004,0,0.007,0 c11.3,0,20.479-2.465,26.541-7.478c12.314-10.181,35.234-29.039,35.081-51.439C287.236,212.71,280.653,201.451,267.749,191.076z" />
                </svg>
                <span>+212661173921</span>
              </a>
            </li>
            <li>
              <a href="mailto:hello@fashionExpertise.agency">
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M8.43008 8.82958C8.30204 8.92344 8.15101 8.97033 8.00002 8.97033C7.84895 8.97033 7.69796 8.92344 7.56992 8.82958L1.45454 4.34498L4.84847e-05 3.27837L0 13.5764C4.84847e-05 13.9781 0.325623 14.3037 0.727271 14.3037L15.2727 14.3036C15.6744 14.3036 16 13.978 16 13.5764V3.27832L14.5454 4.34498L8.43008 8.82958Z"
                    fill="white" />
                  <path d="M7.99998 7.34041L15.0344 2.18169L0.965332 2.18164L7.99998 7.34041Z" fill="white" />
                </svg>
                <span>hello@fashionExpertise.agency</span>
              </a>
            </li>
            <li>
              <a>
                <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g clip-path="url(#clip0_95:406)">
                    <path
                      d="M9.56763 3C6.49761 3 4 5.49761 4 8.56763C4 9.40487 4.18135 10.2095 4.53887 10.9597C4.54607 10.9758 4.55363 10.9915 4.56169 11.0073L8.76832 19.1522C8.92275 19.4511 9.23112 19.639 9.56756 19.639C9.90406 19.639 10.2124 19.4512 10.3668 19.1522L14.5723 11.0095C14.5795 10.9955 14.5863 10.9813 14.5928 10.967C14.9527 10.2146 15.1351 9.40753 15.1351 8.56763C15.1353 5.49761 12.6376 3 9.56763 3ZM9.56763 11.4818C7.96073 11.4818 6.65341 10.1745 6.65341 8.56763C6.65341 6.9608 7.96073 5.65349 9.56763 5.65349C11.1745 5.65349 12.4818 6.9608 12.4818 8.56763C12.4818 10.1745 11.1745 11.4818 9.56763 11.4818Z"
                      fill="white" />
                  </g>
                  <defs>
                    <clipPath id="clip0_95:406">
                      <rect width="19" height="19" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
                <span>9129 East Clinton Lane Fleming <br> Island, FL 32003</span>
              </a>
            </li>
          </ul>
        </div>
      </div>
    </div>
  </footer>
  <script src="js/aos.js"></script>
  <script src="js/ui.js"></script>
</body>

</html>